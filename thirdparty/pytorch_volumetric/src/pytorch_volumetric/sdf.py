...
class ObjectFactory(abc.ABC):
    def __init__(self, name, preload_mesh=None, scale=1.0, vis_frame_pos=(0, 0, 0), vis_frame_rot=(0, 0, 0, 1),
                 plausible_suboptimality=0.001, **kwargs):
        self.name = name
        self.preload_mesh = preload_mesh
    ...
    def precompute_sdf(self):
        ...
        if self.name is None:
            # convert from trimesh mesh
            self._mesh = o3d.geometry.TriangleMesh()
            self._mesh.vertices = o3d.utility.Vector3dVector(np.asanyarray(self.preload_mesh["vertices"]))
            self._mesh.triangles = o3d.utility.Vector3iVector(np.asanyarray(self.preload_mesh["faces"]))
            self._mesh.compute_vertex_normals()
            pass
        elif self.preload_mesh is None:
            ...
            raise RuntimeError("No Pre-load mesh nor mesh name specified. Cannot load mesh.")
        # scale mesh
...
class MeshObjectFactory(ObjectFactory):
    def __init__(self, mesh_name, preload_mesh=None, path_prefix='', **kwargs):
        ...
        super(MeshObjectFactory, self).__init__(mesh_name, preload_mesh, **kwargs)
...
def sample_mesh_points(obj_factory: ObjectFactory = None, num_points=100, seed=0, name="",
    ...
    if cache is not None or os.path.exists(dbpath):
        ...
        if not clean_cache and num_points in cache[name][seed]:
            ...
            #return *res[:-1], cache
            #revise this to be compatiable with the python3.7
            return tuple(res[:-1]) + (cache,)
    else:
        cache = {name: {seed: {}}}
